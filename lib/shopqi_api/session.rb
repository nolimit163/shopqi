  ...
  class Session
    ...
    class << self
      ...
      def temp(domain, token, &block)
        ...
        original_site = ShopqiAPI::Base.site
        begin
          ShopqiAPI::Base.site = session.site
          yield
        ensure
          ShopqiAPI::Base.site = original_site
        end
      end
      ...
      def prepare_url(url)
        ...
        url.concat(".plaveb.ru") unless url.include?('.')   # extend url to myshopify.com if no host is given
      end
