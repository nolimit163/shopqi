  ...
  class Cli < Thor
    ...
    def add(connection)
      ...
      if File.exist?(file)
        ...
        config['domain']   = ask("Domain? (leave blank for #{connection}.plaveb.ru)")
        config['domain']   = "#{connection}.plaveb.ru" if config['domain'].blank?
        config['domain']   = "#{config['domain']}.plaveb.ru" unless config['domain'].match(/[.:]/)
    ...
    def shop_config_dir
      @shop_config_dir ||= File.join(ENV['HOME'], '.shopqi', 'shops')
    end
    ...
    def site_from_config(config)
      ...
      ShopqiAPI::Base.site = "#{protocol}://#{api_key}:#{password}@#{domain}/admin"
    end
